...
public class HashMapOpenAddressing {
	...
	public boolean add(int k, String v) {
		...
		int hash = h;
		if(table[hash].value.equals("") || table[hash].value.equals("Del")) {
			table[hash] = new Element(k, v);
			return true;
		}
	...
	private boolean delete(int hash) {
		table[hash] = new Element(Integer.MIN_VALUE, "Del");
		return true;
	}
